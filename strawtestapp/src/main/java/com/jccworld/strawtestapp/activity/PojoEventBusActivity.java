 ...
 */
public class PojoEventBusActivity extends BaseActivity implements View.OnClickListener {

    private final ProductionInjector injector;

    @Inject @Named("numberPojoConsumableEventBus")
    EventBus<Short> numberConsumableEventBus;

    @Inject @Named("numberPojoStickyEventBus")
    EventBus<Short> numberStickyEventBus;

    @Inject @Named("numberPojoQueuedEventBus")
    EventBus<Short> numberQueuedEventBus;

    private Button numberServiceButton;

    @Inject @Named("wordPojoConsumableEventBus")
    EventBus<String> wordConsumableEventBus;

    @Inject @Named("wordPojoStickyEventBus")
    EventBus<String> wordStickyEventBus;

    @Inject @Named("wordPojoQueuedEventBus")
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.startNumberService) {
            ...
            startService(new Intent(this, NumberPojoService.class));
            ...
            startService(new Intent(this, SentencePojoService.class));
        } else {
            //force null as String (i.e. "null" rather than "" which is what setText will force)
    ...
    class NumberEventListener implements EventBusListener<Short> {
        ...
        public void onEvent(final Short s) {
            String shortString = textView.getText().toString();
